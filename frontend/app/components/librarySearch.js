...
const LibrarySearch= (props,ref) =>{
    ...
    const { data, error,isValidating } = useSWR(
      fetchActive ? `http://localhost:8080/library/search?keyword=${searchTerm}` : null, 
      (path, ...args) =>
  ...
  const handleChange = e => {
    ...
    const handleBlur = e => {
     ...
     return(
        <div className={styles.comboBoxContainer}><Autocomplete className="combobox" options={filteredLibraries} disablePortal renderInput={(params) => <TextField {...params} onBlur={handleBlur} onChange={handleChange} inputRef={ref} label="Library" />} placeholder="Library"/><Button className={styles.linkButton} >Add new</Button> </div>
     );
}
